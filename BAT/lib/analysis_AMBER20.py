...
import math
import numpy as np 
from akima.akima import interpolate
            ...
            with open("md-02.out", "r") as fin:
              ...
              for line in fin:
                ...
                if 'DV/DL  = ' in line and s == 1:
                  n = n+1
                  splitdata = line.split()
                  valDL = float(splitdata[2])
                if 'Lambda-dependent restraint contribution to dV/Dl: Total = ' in line and s == 1:
                  splitdata = line.split()
                  valDI = float(splitdata[7])
                  valX = valDL - valDI 
                  data.append(valX)
                  dvdl.write('%5d   %9.4f\n' % (n, valX))
                  s = 0
    ...
    for i in range(0, len(components)):
      ...
      if comp == 'a' or comp == 'l' or comp == 't' or comp == 'c' or comp == 'r' or comp == 'u':
        ...
        if dd_type == 'ti':
          rest_file = 'dvdl.dat'
          mode = 'all'
          fe_dd(comp, pose, mode, lambdas, weights, dd_type, rest_file, temperature)
        elif dd_type == 'mbar':
...
def fe_mbar(comp, pose, mode, rest_file, temperature):
    ...
    print  "Running MBAR... " 
    mbar = pymbar.MBAR(Upot, Neff, verbose = True, method = 'adaptive', initialize = 'BAR')
